...
client = Bot(description="Here is some commands for you", command_prefix="b!", pm_help = False) #add_your_bot_description_with_prefix_here
...
async def on_ready():
	...
	print('--------')
	print('--------')
	print('Started DP') #add_your_bot_name_here
	return await client.change_presence(game=discord.Game(name='BS艾OFFICIAL || b!help')) #add_your_bot_status_here
...
async def dm(ctx, user: discord.Member, *, msg: str):
    try:
        ...
        await client.say("Error :x:. Make sure your message is shaped in this way: b!dm [tag person] [msg]")
...
async def server(ctx):
    ...
    await client.say('https://discord.gg/m5WwC72')
...
async def info(ctx):
    ...
    await client.say('***THIS BOT IS MADE BY Dp♡#1471 AND IF YOU NEED TO INVITE THIS BOT SO WRITE `b!botinvite` THANKS***')

@client.command(pass_context = True)

async def botinvite(ctx):

    await client.say('https://discordapp.com/api/oauth2/authorize?client_id=553105968712646696&permissions=8&scope=bot')
...
async def avatar(ctx, user: discord.Member=None):
    if user is None:
        ...
        embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/539446050885337109/553098032590225408/Screenshot_2019-03-07-11-43-35-603_com.miui.gallery.png')
...
async def ownerinfo(ctx):
    ...
    embed.add_field(name="2nd owner", value="DP♡#1471 he code the bot", inline=True)
    await client.say(embed=embed)
